...
RSpec.describe Team, type: :model do
  ...
  it 'will not be valid with a missing team name' do
    team = Team.new(name: nil, event_id: event.id, closed: false)
    expect(team).not_to be_valid
  end

  it 'will not be valid with too short of a team name' do
    team = Team.new(name: '', event_id: event.id, closed: false)
    expect(team).not_to be_valid
  end

  it 'will not be valid with too long of a team name' do
    team = Team.new(name: '0' * 65, event_id: event.id, closed: false)
    expect(team).not_to be_valid
  end

  it 'will not be valid with a missing event' do
    team = Team.new(name: 'Test Team', event_id: nil, closed: false)
    expect(team).not_to be_valid
  end

  it 'will not be valid with an invalid closed boolean' do
